...
def _call_with_timeout(fn: Callable, args: tuple, timeout: float) -> tuple[str, Any]:
    """Run ``fn(*args)`` in a daemon thread, return ("ok"|"err"|"timeout", value)."""
...
def run_evaluation(
    ...
    try:
        ...
        status, value = _call_with_timeout(
            _run_search_algorithm, (program, config, G),
            timeout=SEARCH_ALGORITHM_TIMEOUT_SECS,
        )
        if status == "timeout":
            return False, 0.0, 0.0, (
                f"search_algorithm timed out (>{SEARCH_ALGORITHM_TIMEOUT_SECS:.0f}s) "
                f"on {os.path.basename(config_file)}"
            ), {}
        if status == "err":
            raise value  # re-raise so the outer except formats it consistently
        topology = value
